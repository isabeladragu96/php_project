<?php
	...
	include("connection.php");
		...
		$query_toate_numele = "SELECT A.`NumeEveniment` FROM `eveniment` A INNER JOIN `categorie` B 
					 ...
					 WHERE B.`NumeCategorie` = 'Expozitie'";
		...
		$query_toate_duratele = "SELECT B.`Data`, B.`Ora`, B.`Durata` FROM `eveniment` A INNER JOIN `durata` B 
						 ...
						 WHERE C.NumeCategorie = 'Expozitie' "  ;
	...
	if (array_key_exists("submit", $_POST)) {
		...
		$query = "DELETE
				  FROM `eveniment`
				  WHERE `NumeEveniment` = 'Expozitie de Arta'";
		...
		$query_sector_nume = "SELECT A.`NumeEveniment` FROM `eveniment` A INNER JOIN `categorie` B 
					 ON A.IdCategorie = B.IdCategorie 
					 WHERE B.`NumeCategorie` = 'Expozitie' AND '".$_POST['sector']."' = (SELECT C.`Sectorul` 
																						 FROM `locatie` C 
		...
		$query_sector_durata = "SELECT B.`Data`, B.`Ora`, B.`Durata` FROM `eveniment` A INNER JOIN `durata` B 
					 ...
					 WHERE 'Expozitie' = (SELECT C.NumeCategorie 
										FROM `categorie` C
...
<style type="text/css">
		...
		html { 
		  background: url(expozitie-fundal.jpg) no-repeat center center fixed; 
...
<div class = "container">
	...
	<form method="post" class="form-inline" id="form">
		...
		<select class="custom-select mb-2 mr-sm-2 mb-sm-0" name = "sector" >
		...
		<?php 
			...
			while($i < sizeof($shows) - 1) { 
				...
				echo '<div id="nume">'.$shows[$i].
					...
					"<br>Locatie: ".$location_one[$i]."<br>Strada: ".$street_one[$i].", ".$number_one[$i]."<br><br>";
